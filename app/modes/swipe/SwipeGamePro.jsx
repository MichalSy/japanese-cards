...
export default function SwipeGamePro({ contentType, groupId, cardCount }) {
  ...
  const handleButtonClick = (isCorrect) => {
    if (buttonClickRef.current) {
      buttonClickRef.current(isCorrect)
    }
  }
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#0f172a', color: 'rgba(255,255,255,0.6)' }}>
  ...
  if (error) {
    return (
      <div style={{ padding: '20px', height: '100vh', backgroundColor: '#0f172a' }}>
        <div style={{ padding: '20px', backgroundColor: 'rgba(239, 68, 68, 0.1)', borderRadius: '16px', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
          Fehler: {error}
        </div>
      </div>
    )
  ...
  if (game.gameState === 'finished') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#0f172a', backgroundImage: 'url(/japanese-cards/images/swipe-bg.png)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
          <div style={{ background: 'rgba(30, 41, 59, 0.8)', backdropFilter: 'blur(20px)', borderRadius: '20px', border: '1px solid rgba(236, 72, 153, 0.2)', padding: '32px 24px', textAlign: 'center', marginBottom: '20px' }}>
            <h2 style={{ fontSize: '28px', fontWeight: '700', color: 'white', margin: '0 0 24px 0' }}>Spiel beendet! 🎉</h2>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '40px' }}>
              <div>
                <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.5)', margin: 0 }}>Richtig</p>
                <p style={{ fontSize: '36px', fontWeight: '700', color: '#10b981', margin: '8px 0 0 0' }}>{game.stats.correct}/{total}</p>
              </div>
              <div>
                <p style={{ fontSize: '14px', color: 'rgba(255,255,255,0.5)', margin: 0 }}>Prozent</p>
                <p style={{ fontSize: '36px', fontWeight: '700', color: '#ec4899', margin: '8px 0 0 0' }}>{percentage}%</p>
              </div>
            </div>
          </div>
          {game.stats.mistakes.length > 0 && (() => {
            ...
            const seen = new Set()
            game.stats.mistakes.forEach(m => {
              const char = m.realCard?.character || m.realCard?.word
              if (char && !seen.has(char)) { seen.add(char); uniqueMistakes.push(m) }
            })
            return (
              <div>
                <h3 style={{ fontSize: '16px', fontWeight: '600', color: 'rgba(255,255,255,0.7)', marginBottom: '16px' }}>📚 Nochmal üben</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '12px' }}>
                  {uniqueMistakes.map((m, i) => (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 16px', backgroundColor: 'rgba(30, 41, 59, 0.6)', backdropFilter: 'blur(10px)', borderRadius: '16px', border: '1px solid rgba(236, 72, 153, 0.2)' }}>
                      <span style={{ fontSize: '48px', fontWeight: '300', color: 'white', lineHeight: 1, marginBottom: '8px' }}>{m.realCard?.character || m.realCard?.word}</span>
                      <span style={{ fontSize: '16px', color: '#ec4899', fontWeight: '600' }}>{m.realCard?.romaji}</span>
                    </div>
                  ))}
                </div>
              </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', padding: '8px 20px 16px' }}>
        <span style={{ fontSize: '18px', fontWeight: '500', color: 'white', letterSpacing: '0.3px' }}>
      ...
      {toast && (
        <div style={{ position: 'fixed', top: '140px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000, opacity: toastVisible ? 1 : 0, transition: 'opacity 0.3s', pointerEvents: 'none' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 24px', backgroundColor: toast.isCorrect ? 'rgba(16, 185, 129, 0.9)' : 'rgba(239, 68, 68, 0.9)', backdropFilter: 'blur(10px)', borderRadius: '16px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
      ...
      {/* Progress Bar */}
      <div style={{ padding: '8px 24px', paddingBottom: 'max(20px, env(safe-area-inset-bottom))' }}>
        ...
        <div style={{ height: '4px', backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: '2px', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg, #ec4899, #8b5cf6)', borderRadius: '2px', transition: 'width 0.3s' }} />
        </div>
      </div>
    </div>
  )
