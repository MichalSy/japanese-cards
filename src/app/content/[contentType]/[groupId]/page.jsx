...
export default function GameModeSelector({ params }) {
  ...
  const groupName = categoryConfig?.groups?.find(g => g.id === groupId)?.name ?? groupId
              ...
              {gameModes.map(mode => (
                <Card key={mode.id} interactive onClick={() => router.push(`/game/${contentType}/${groupId}/${mode.id}?cards=${cardCount}`)}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <div style={{ width: '52px', height: '52px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(236,72,153,0.12)', border: '1px solid rgba(236,72,153,0.2)', borderRadius: '14px' }}>
                      <ModeIcon id={mode.id} />
                    </div>
                    <div style={{ flex: 1 }}>
