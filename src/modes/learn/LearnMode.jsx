...
import LearnCardSentence from './LearnCardSentence'
import LearnSentenceQuiz from './LearnSentenceQuiz'
...
export default function LearnMode({ lesson, cards, lang }) {
  ...
  const lessonCards = useMemo(() => {
    const learningCards = cards.filter(card => card.card_type !== 'quiz_4_option' && card.card_type !== 'sentence_quiz')
    const quizCards = randomShuffle(cards.filter(card => card.card_type === 'quiz_4_option' || card.card_type === 'sentence_quiz'))
  ...
  const isQuiz = card?.card_type === 'quiz_4_option' || card?.card_type === 'sentence_quiz'
  ...
  const quizCardIndices = useMemo(() =>
    lessonCards.reduce((acc, c, i) => { if (c.card_type === 'quiz_4_option' || c.card_type === 'sentence_quiz') acc.push(i); return acc }, []),
    [lessonCards]
  )
  ...
  const renderCardContent = () => {
    if (!card) return null
    if (card.card_type === 'sentence_quiz') return (
      <LearnSentenceQuiz
        card={card}
        lang={lang}
    ...
    if (card.card_type === 'quiz_4_option') return renderQuizContent()
    if (card.card_type === 'sentence') return <LearnCardSentence card={card} lang={lang} />
