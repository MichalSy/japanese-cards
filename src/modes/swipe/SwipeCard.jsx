...
export default function SwipeCard({ card, index, isActive, onSwipe, correctAnswer }) {
  ...
  const [, setExitDirection] = useState(null)
  ...
  return (
    <div
      ...
      style={{
        ...
        height: 'calc(100% - 48px)', maxHeight: '500px',
        background: 'var(--color-surface)', borderRadius: '16px',
        border: '1px solid var(--color-surface-light)',
        zIndex: 100 - index, cursor: isActive && !isDragging ? 'grab' : isDragging ? 'grabbing' : 'default',
    ...
    >
      <div style={{
        ...
        <span style={{ fontSize: '36px', opacity: isSwipingLeft ? swipeProgress : 0, transform: `scale(${0.6 + swipeProgress * 0.4})`, transition: isDragging ? 'none' : 'all 0.2s' }}>✗</span>
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: '36px', opacity: isSwipingRight ? swipeProgress : 0, transform: `scale(${0.5 + swipeProgress * 0.5})`, transition: isDragging ? 'none' : 'all 0.2s' }}>✓</span>
      </div>

      <div style={{
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        justifyContent: 'center', flex: 1, padding: '40px 24px', gap: '20px',
      }}>
        <div style={{ fontSize: 'clamp(100px, 28vw, 180px)', fontWeight: '200', lineHeight: 1, color: 'var(--color-text-primary)', textAlign: 'center' }}>
          {character}
        </div>
        {(card.shownRomaji || card.romaji) && (
          <div style={{ fontSize: '28px', color: 'var(--color-primary)', fontWeight: '500', letterSpacing: '3px', textTransform: 'lowercase' }}>
            {card.shownRomaji || card.romaji}
          </div>
        )}
      </div>

      <div style={{ display: 'flex', padding: '20px 24px 28px', gap: '16px', opacity: swipeState === 'exit' ? 0 : 1 }}>
        <button
          onClick={() => !swipeState && triggerSwipe(false)}
          disabled={swipeState === 'exit'}
          style={{
            flex: 1, padding: '16px 20px', borderRadius: '100px',
            backgroundColor: 'rgba(239,68,68,0.08)', border: 'none',
            color: '#f87171', fontSize: '15px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s',
          }}
        ...
        <button
          onClick={() => !swipeState && triggerSwipe(true)}
          disabled={swipeState === 'exit'}
          style={{
            flex: 1, padding: '16px 20px', borderRadius: '100px',
            backgroundColor: 'rgba(16,185,129,0.08)', border: 'none',
            color: '#34d399', fontSize: '15px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s',
          }}
