...
export default function SwipeGame({ contentType, groupId, cardCount }) {
  ...
  if (error) {
    return (
      <AppContent>
        <div style={{ padding: 'var(--spacing-4)', backgroundColor: 'rgba(239,68,68,0.1)', borderRadius: 'var(--radius-md)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.3)' }}>
          Fehler: {error}
        </div>
      </AppContent>
    )
  ...
  if (game.gameState === 'finished') {
    ...
    game.stats.mistakes.forEach(mistake => {
      ...
      if (char && !seen.has(char)) { seen.add(char); uniqueMistakes.push(mistake) }
    })
        ...
        <div className="space-y-6">
          <Card>
            <div style={{ textAlign: 'center' }}>
              <h2 className="text-2xl font-bold" style={{ color: 'var(--color-text-primary)', margin: '0 0 var(--spacing-4) 0' }}>Spiel beendet! 🎉</h2>
              <div className="grid-2">
                <div>
                  <p className="text-sm text-tertiary" style={{ margin: 0 }}>Richtig</p>
                  <p className="text-3xl font-bold" style={{ color: '#10b981', margin: 0 }}>{game.stats.correct}/{total}</p>
                </div>
                <div>
                  <p className="text-sm text-tertiary" style={{ margin: 0 }}>Prozentsatz</p>
                  <p className="text-3xl font-bold" style={{ color: 'var(--color-primary)', margin: 0 }}>{percentage}%</p>
                </div>
              </div>
            </div>
          </Card>
          ...
          {uniqueMistakes.length > 0 && (
            <div>
              <h3 className="text-base font-medium" style={{ color: 'var(--color-text-secondary)', marginBottom: 'var(--spacing-4)' }}>📚 Nochmal üben</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '12px' }}>
                {uniqueMistakes.map((mistake, idx) => (
                  <div key={idx} style={{
                    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                    padding: '20px 16px', backgroundColor: 'var(--color-surface)',
                    borderRadius: '16px', border: '1px solid var(--color-surface-light)',
                  }}>
                    <span style={{ fontSize: '48px', fontWeight: '300', color: 'var(--color-text-primary)', lineHeight: 1, marginBottom: '8px' }}>
                      {mistake.realCard?.character || mistake.realCard?.word || '?'}
                    </span>
                    <span style={{ fontSize: '16px', color: 'var(--color-primary)', fontWeight: '600' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', position: 'relative' }}>
      <div style={{ padding: '12px 16px', textAlign: 'center', borderBottom: '1px solid var(--color-surface-light)', backgroundColor: 'var(--color-bg-secondary)' }}>
        <span style={{ fontSize: '14px', fontWeight: '500', color: 'var(--color-text-secondary)', letterSpacing: '0.3px' }}>
      ...
      {toast && (
        <div key={toast.id} style={{
          ...
          zIndex: 1000, opacity: toastVisible ? 1 : 0, transition: 'opacity 0.3s ease-out', pointerEvents: 'none',
        }}>
          <div style={{
      ...
      <div style={{ padding: '12px 16px', paddingBottom: 'max(12px, env(safe-area-inset-bottom))', backgroundColor: 'var(--color-surface)', borderTop: '1px solid var(--color-surface-light)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
          <div style={{ flex: 1, height: '6px', backgroundColor: 'var(--color-surface-light)', borderRadius: '3px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg, var(--color-primary), var(--color-secondary))', borderRadius: '3px', transition: 'width 0.3s ease' }} />
          </div>
          <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--color-text-secondary)', minWidth: '60px', textAlign: 'right' }}>
